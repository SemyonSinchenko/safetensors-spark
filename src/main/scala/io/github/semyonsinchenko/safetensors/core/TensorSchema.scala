...
/** The canonical Tensor Struct type used for all read, write, and Catalyst expression operations in
  * this connector.
  *
  * Schema: StructType( StructField("data", BinaryType, nullable = false), StructField("shape",
  * ArrayType(IntegerType), nullable = false), StructField("dtype", StringType, nullable = false) )
  *
  *   - data: raw little-endian bytes of the tensor, exactly as stored in the safetensors byte
  *     buffer.
  *   - shape: dimension sizes (e.g. [3, 224, 224]). Empty array [] for scalars.
  *   - dtype: one of the SafetensorsDtype string representations.
  *
  * Rationale: BinaryType is the only Spark type that can losslessly represent all safetensors
  * dtypes including BF16 and F16, for which Spark has no native numeric type.
  */
object TensorSchema {
  ...
  /** Returns true if the given StructType matches the canonical Tensor Struct (field names and
    * types must match exactly; nullability is not checked).
    */
  ...
  /** Returns true if the given DataType is a numeric ArrayType that the connector can encode into
    * tensor bytes.
    */
