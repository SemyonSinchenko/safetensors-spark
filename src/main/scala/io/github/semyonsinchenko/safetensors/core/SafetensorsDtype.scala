 ...
 * `([UIF])(8|16|32|64|128|256)`, which does NOT match "BF16". BF16 is a known
 * special case handled outside the JSON schema by the safetensors library.
 * This connector hardcodes BF16 as a valid dtype, bypassing the schema regex.
 * See §1.1 of the project specification (AGENTS.md) for details.
 */
...
object SafetensorsDtype {
  ...
  def fromString(s: String): Either[String, SafetensorsDtype] =
    all.find(_.name == s).toRight(s"Unknown safetensors dtype: '$s'. Valid values: ${all.map(_.name).mkString(", ")}")
