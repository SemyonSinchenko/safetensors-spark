...
/** In-memory representation of a parsed safetensors file header.
  ...
  *
  * The JSON header is a map from tensor name to TensorInfo, with the optional reserved key
  * "__metadata__" for free-form string-to-string metadata.
  *
  * data_offsets are relative to the start of the byte buffer (i.e. NOT to the start of the file).
  * Absolute file offset of tensor data = 8 + N + data_offsets.begin
  *
  * The byte buffer must be fully indexed (no holes between tensors). Tensor values are stored in
  * little-endian, C/row-major order.
  */
...
/** Parsed representation of a complete safetensors file header.
  *
  * @param tensors
  *   Ordered map from tensor name to TensorInfo. Ordering follows the original JSON key order.
  * @param metadata
  *   Optional free-form string-to-string metadata (from the "__metadata__" key).
  * @param headerSize
  *   Size of the JSON header in bytes (= N from the 8-byte prefix). The byte buffer starts at file
  *   offset 8 + headerSize.
  */
final case class SafetensorsHeader(
    tensors: ListMap[String, TensorInfo],
  ...
  /** Tensor names in JSON key insertion order. */
