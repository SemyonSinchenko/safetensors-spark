...
/**
 * Parses the binary safetensors header from a ByteBuffer positioned at the
 * start of the file.
 *
 ...
 *
 * This parser reads only the 8 + N header bytes. Tensor data bytes are NOT
 * read here — callers use the DataOffsets from TensorInfo to locate them.
 */
object SafetensorsHeaderParser {
   ...
   *
   * The buffer must be positioned at the very start of the file (offset 0)
   * and must have at least 8 bytes available. The buffer's byte order will
   * be set to LITTLE_ENDIAN during parsing.
   *
   * @param buf  ByteBuffer backed by the file (e.g. a MappedByteBuffer).
   * @return     Parsed SafetensorsHeader.
  ...
  private def parseJson(json: String, headerSize: Long): SafetensorsHeader = {
    ...
    val tensors: Map[String, TensorInfo] = root
      .view.filterKeys(_ != "__metadata__").toMap
      .map { case (name, value) =>
