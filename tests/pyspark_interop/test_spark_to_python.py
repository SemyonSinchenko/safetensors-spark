...
def f32_to_f16_truncate(f32_array: np.ndarray) -> np.ndarray:
    ...
    normal_result = (
        (sign[normal].astype(np.uint16) << 15) | (exp16_clipped << 10) | mant16
    )
    # underflow → zero; overflow → inf
    normal_result[underflow] = sign[normal][underflow].astype(np.uint16) << 15
    normal_result[overflow] = (
        sign[normal][overflow].astype(np.uint16) << 15
    ) | np.uint16(0x7C00)
...
def float_arrays_df(spark):
    ...
    rows = [
        Row(
            image=Row(
                data=make_f32_bytes([1.0, 2.0, 3.0, 4.0]), shape=[2, 2], dtype="F32"
            ),
            label=Row(data=make_f32_bytes([0.0]), shape=[1], dtype="F32"),
        ),
        Row(
            image=Row(
                data=make_f32_bytes([5.0, 6.0, 7.0, 8.0]), shape=[2, 2], dtype="F32"
            ),
...
def test_manifest_is_written(spark, float_arrays_df, tmp_path: Path):
    ...
    (
        float_arrays_df.write.format("safetensors")
        .option("batch_size", "2")
        .option("dtype", "F32")
        .mode("overwrite")
        .save(out_dir)
    ...
    total_bytes_on_disk = sum(p.stat().st_size for p in out_path.glob("*.safetensors"))
...
def test_batch_mode_tail_strategy_drop(spark, tmp_path: Path):
    ...
    rows = [
        Row(
            tensor=Row(
                data=make_f32_bytes([float(i), 0.0, 0.0, 0.0]), shape=[4], dtype="F32"
            )
        )
        for i in range(NUM_ROWS)
    ]
...
def test_name_col_basic(spark, tmp_path: Path):
    ...
    rows = [
        Row(
            tensor_key="row_0",
            tensor=Row(
                data=make_f32_bytes([1.0, 2.0, 3.0, 4.0]), shape=[4], dtype="F32"
            ),
        ),
        Row(
            tensor_key="row_1",
            tensor=Row(
                data=make_f32_bytes([5.0, 6.0, 7.0, 8.0]), shape=[4], dtype="F32"
            ),
        ),
        Row(
            tensor_key="row_2",
...
def test_duplicates_strategy_fail(spark, tmp_path: Path):
    ...
    full_message = str(exc_info.value) + "".join(str(a) for a in exc_info.value.args)
    assert any(
...
def test_duplicates_strategy_last_win(spark, tmp_path: Path):
    ...
    np.testing.assert_array_almost_equal(
        all_tensors["shared_key__tensor"], [99.0, 100.0]
    )
...
def test_name_col_multi_column(spark, tmp_path: Path):
    ...
    np.testing.assert_array_almost_equal(
        all_tensors["model_0__weights"], [0.1, 0.2, 0.3]
    )
...
def test_name_col_custom_separator(spark, tmp_path: Path):
    ...
    assert "row_0/tensor" in found_keys, (
        "Custom separator '/' should be used in tensor key"
    )
...
def test_tensor_index_file_name_routing(spark, tmp_path: Path):
    ...
    shard_names_on_disk = {
        p.name for p in (tmp_path / "index_routing_output").glob("*.safetensors")
    }
    ...
    assert len(pairs) == len(set(pairs)), (
        "Duplicate (tensor_key, file_name) pairs in index"
    )
...
def test_overwrite_mode_removes_stale_files(spark, tmp_path: Path):
    ...
    spark.createDataFrame(rows_first, schema).coalesce(1).write.format(
        "safetensors"
    ).option("batch_size", "2").mode("overwrite").save(out_dir)

    first_shards = {p.name for p in Path(out_dir).glob("*.safetensors")}
    assert len(first_shards) == 3

    spark.createDataFrame(rows_second, schema).coalesce(1).write.format(
        "safetensors"
    ).option("batch_size", "2").mode("overwrite").save(out_dir)
...
def test_random_f32_array_round_trip(spark, tmp_path: Path):
    ...
    np.testing.assert_array_equal(
        t, source_arrays, err_msg="F32 round-trip: output does not match input exactly"
    )
...
def test_spark_read_back_batch_mode_output(spark, tmp_path: Path):
    ...
    df_read = (
        spark.read.format("safetensors").option("inferSchema", "true").load(out_dir)
    )
...
def test_multi_column_multi_dtype_write(spark, tmp_path: Path):
    ...
    rows = [
        Row(
            ...
            attention_mask=Row(
                data=make_f32_bytes([1.0] * 128), shape=[128], dtype="F32"
            ),
